...
To run the application you would first have to run the Docker Postgres container which would also map the port 5432 of my container to my servers port 5432.
Next up you would like to package the maven project to a jar file and after that run that jar file.
...
In order to close the application and remove the container you would have to run the following commands:
    1)jps
    From the given outcome check what is the process id (PID) of the application running named app.jar.
    2)taskkill /F /PID *the PID of that application*
    3)docker ps
    From the given outcome check what is the CONTAINER ID of the container running named concise.
